...
for i in range(len(grid_corner_lon)):
    ...
    ax1.add_patch(plt.Polygon(cell, fill=None, edgecolor='black', linewidth=0.5, transform=ccrs.PlateCarree()))
...
for i in range(len(grid_corner_lon)):
    ...
    cell = list(zip(transformed_poly[:, 0], transformed_poly[:, 1]))
    ax2.add_patch(plt.Polygon(cell, fill=None, edgecolor='purple', linewidth=0.5))
